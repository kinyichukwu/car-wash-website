import Navbar from "./components/Navbar";
import Home from "./components/Home";
import productImage from "./assets/carwash-removebg.png";

const WhatsAppLink = () => {
  // Replace with the desired phone number and message (optional)

  return (
    <div>
      <p>Click the link below to open WhatsApp:</p>
      <a href={whatsappUrl} target="_blank" rel="noopener noreferrer">
        Open WhatsApp
      </a>
    </div>
  );
};

function App() {
  const phoneNumber = "+2348082886416";
  const message = "Hello, I will like to purchase Supreme Car Wash ";

  // Generate the WhatsApp URL
  const whatsappUrl = `https://wa.me/${phoneNumber}?text=${encodeURIComponent(
    message
  )}`;

  return (
    <>
      <Navbar />
      <Home />
      <div className="flex flex-wrap my-8 md:py-4 justify-between items-center max-w-screen-2xl mx-auto md:px-10 px-3">
        <img src={productImage} alt="" className=" md:w-[15rem] w-[30%]" />
        <img src={productImage} alt="" className=" md:w-[15rem] w-[30%]" />
        <img src={productImage} alt="" className=" md:w-[15rem] w-[30%]" />
      </div>
      <p className="text-blue-600 font-semibold md:text-[2.156rem] text-lg text-center max-w-screen-2xl mx-auto md:px-10 px-3">
        Buy Now for <span className=" line-through text-red-500">₦ 5,000</span>{" "}
        <span className="">₦ 2,000 ONLY.</span>{" "}
      </p>

      <div className="max-w-screen-2xl mx-auto md:px-10 px-3 text-center md:w-[70%] md:mt-36 mt-12 mb-[2.38rem]">
        <h1 className=" md:text-4xl text-2xl text-[#0B132A] font-medium mt-7 ">
          Supreme Car Paint Protector
        </h1>
        <p className=" text-[#0B132A] md:text-xl text-lg font-medium md:mt-7 mt-3">
          Product Description
        </p>
        <p className="text-[#4F5665] text-lg md:mt-7 mt-3">
          "Introducing our premium car wash soap, the secret to a showroom shine
          at your fingertips. With its advanced formula, our soap effortlessly
          cleans and revives your ride, leaving it glistening with pride.
          Experience the joy of a spotless car, turning heads wherever you go.
          Elevate your car wash routine with our trusted solution. Grab yours
          today and make your car a reflection of perfection."
        </p>
      </div>

      <div className="max-w-screen-2xl mx-auto md:px-10 px-3 flex justify-center items-center md:gap-24 flex-col md:flex-row">
        <div className=" text-center">
          <span className=" line-through font-semibold text-[2.156rem] text-center text-[#0B132A]">
            ₦ 5,000
          </span>{" "}
          <br />
          <span className="text-[#0B132A]">Original Price</span>
        </div>

        <div className=" text-center mt-5 md:mt-0">
          <span className=" font-semibold text-[2.156rem] text-center text-blue-600">
            ₦ 2,000
          </span>{" "}
          <br />
          <span className=" text-blue-600">Discount Offer</span>
        </div>
      </div>

      <div className="md:mt-28 max-w-screen-2xl mx-auto mt-12">
    
          <a
            href={whatsappUrl}
            target="_blank"
            rel="noopener noreferrer"
            className="py-3 w-fit px-6 no-underline text-white mx-auto block bg-blue-500 rounded-2xl text-base font-medium  hover:bg-blue-600   active:bg-blue-500 animate-bounce"
          >
            Buy Now
          </a>
     

        <img src={productImage} alt="" className=" mt-14 w-[18rem] mx-auto" />
      </div>

      <div className="max-w-screen-2xl mx-auto md:px-10 px-3 text-center md:w-[70%] md:mt-36 mt-12 mb-[2.38rem]">
        <h1 className=" text-4xl text-[#0B132A] font-medium my-7 ">
          Contact Info
        </h1>
        <a
        href={whatsappUrl}
        target="_blank"
        rel="noopener noreferrer"
        className="py-3 w-fit px-6 no-underline text-white mx-auto block bg-blue-500 rounded-2xl text-base font-medium  hover:bg-blue-600   active:bg-blue-500 "
      >
       Contact Us
      </a>
      </div>

      <section class="bg-blue-700 max-w-screen-2xl mx-auto">
        <div class="max-w-screen-2xl px-4 py-12 mx-auto space-y-8 overflow-hidden sm:px-6 lg:px-8">
          <nav class="flex flex-wrap justify-center md:mx-5 my-2">
            <div class="px-5 py-2">
              <a
                href="#"
                class="text-base leading-6 text-white hover:text-gray-900"
              >
                About
              </a>
            </div>

            <div class="px-5 py-2">
              <a
                href="#"
                class="text-base leading-6 text-white hover:text-gray-900"
              >
                Pricing
              </a>
            </div>
            <div class="px-5 py-2">
              <a
                href="#"
                class="text-base leading-6 text-white hover:text-gray-900"
              >
                Contact
              </a>
            </div>
          </nav>
          <div class="flex justify-center mt-8 space-x-6">
            <a href="#" class="text-white hover:text-white">
              <span class="sr-only">Facebook</span>
              <svg
                class="w-6 h-6"
                aria-hidden="true"
                fill="currentColor"
                viewBox="0 0 24 24"
              >
                <path
                  fill-rule="evenodd"
                  d="M22 12c0-5.523-4.477-10-10-10S2 6.477 2 12c0 4.991 3.657 9.128 8.438 9.878v-6.987h-2.54V12h2.54V9.797c0-2.506 1.492-3.89 3.777-3.89 1.094 0 2.238.195 2.238.195v2.46h-1.26c-1.243 0-1.63.771-1.63 1.562V12h2.773l-.443 2.89h-2.33v6.988C18.343 21.128 22 16.991 22 12z"
                  clip-rule="evenodd"
                ></path>
              </svg>
            </a>
            <a href="#" class="text-white hover:text-white">
              <span class="sr-only">Instagram</span>
              <svg
                class="w-6 h-6"
                aria-hidden="true"
                fill="currentColor"
                viewBox="0 0 24 24"
              >
                <path
                  fill-rule="evenodd"
                  d="M12.315 2c2.43 0 2.784.013 3.808.06 1.064.049 1.791.218 2.427.465a4.902 4.902 0 011.772 1.153 4.902 4.902 0 011.153 1.772c.247.636.416 1.363.465 2.427.048 1.067.06 1.407.06 4.123v.08c0 2.643-.012 2.987-.06 4.043-.049 1.064-.218 1.791-.465 2.427a4.902 4.902 0 01-1.153 1.772 4.902 4.902 0 01-1.772 1.153c-.636.247-1.363.416-2.427.465-1.067.048-1.407.06-4.123.06h-.08c-2.643 0-2.987-.012-4.043-.06-1.064-.049-1.791-.218-2.427-.465a4.902 4.902 0 01-1.772-1.153 4.902 4.902 0 01-1.153-1.772c-.247-.636-.416-1.363-.465-2.427-.047-1.024-.06-1.379-.06-3.808v-.63c0-2.43.013-2.784.06-3.808.049-1.064.218-1.791.465-2.427a4.902 4.902 0 011.153-1.772A4.902 4.902 0 015.45 2.525c.636-.247 1.363-.416 2.427-.465C8.901 2.013 9.256 2 11.685 2h.63zm-.081 1.802h-.468c-2.456 0-2.784.011-3.807.058-.975.045-1.504.207-1.857.344-.467.182-.8.398-1.15.748-.35.35-.566.683-.748 1.15-.137.353-.3.882-.344 1.857-.047 1.023-.058 1.351-.058 3.807v.468c0 2.456.011 2.784.058 3.807.045.975.207 1.504.344 1.857.182.466.399.8.748 1.15.35.35.683.566 1.15.748.353.137.882.3 1.857.344 1.054.048 1.37.058 4.041.058h.08c2.597 0 2.917-.01 3.96-.058.976-.045 1.505-.207 1.858-.344.466-.182.8-.398 1.15-.748.35-.35.566-.683.748-1.15.137-.353.3-.882.344-1.857.048-1.055.058-1.37.058-4.041v-.08c0-2.597-.01-2.917-.058-3.96-.045-.976-.207-1.505-.344-1.858a3.097 3.097 0 00-.748-1.15 3.098 3.098 0 00-1.15-.748c-.353-.137-.882-.3-1.857-.344-1.023-.047-1.351-.058-3.807-.058zM12 6.865a5.135 5.135 0 110 10.27 5.135 5.135 0 010-10.27zm0 1.802a3.333 3.333 0 100 6.666 3.333 3.333 0 000-6.666zm5.338-3.205a1.2 1.2 0 110 2.4 1.2 1.2 0 010-2.4z"
                  clip-rule="evenodd"
                ></path>
              </svg>
            </a>
          </div>
          <p class="mt-8 text-base leading-6 text-center text-white">
            © 2021 SomeCompany, Inc. All rights reserved.
          </p>
        </div>
      </section>
    </>
  );
}

export default App;
